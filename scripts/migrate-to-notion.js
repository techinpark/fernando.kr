...
function splitIntoBlocks(text, maxLength = 2000) {
  ...
  while (currentText.length > 0) {
    ...
    if (currentText.length > maxLength) {
      ...
      const breakPoint = lastNewline > maxLength * 0.8 ? lastNewline : lastSpace > maxLength * 0.8 ? lastSpace : maxLength
...
async function migrateMarkdownToNotion(filePath) {
  ...
  try {
    ...
    return { success: true, slug, title: frontmatter.title }
  } catch (error) {
    console.error(`  ❌ 실패: ${error.message}\n`)
    return { success: false, slug, title: frontmatter?.title, error: error.message }
  }
}
  ...
  for (const category of categories) {
    ...
    for (const file of files) {
      ...
      if (existingSlugs.has(slug)) {
        console.log(`⏭️  건너뜀: ${slug} (이미 존재)\n`)
        results.skipped.push({ slug, filename })
        continue
      }

      const result = await migrateMarkdownToNotion(filePath)
